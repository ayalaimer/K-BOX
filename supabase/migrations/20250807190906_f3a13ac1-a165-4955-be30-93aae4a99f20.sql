-- Stage 1: Database setup for booking management system
-- Note: Uses existing function public.update_updated_at_column()

-- 1) Create roles enum and table for admin authorization
DO $$
BEGIN
...
-- Allow admins to manage roles; users can view their own roles
...
-- 2) Business hours table (supports regular days and special dates)
...
-- Unique per regular weekday and per special date
...
-- Public can read hours; only admins can modify
CREATE POLICY IF NOT EXISTS "Anyone can view business hours"
ON public.business_hours
FOR SELECT
USING (true);

CREATE POLICY IF NOT EXISTS "Admins can modify business hours"
ON public.business_hours
FOR INSERT
TO authenticated
WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY IF NOT EXISTS "Admins can update business hours"
ON public.business_hours
FOR UPDATE
TO authenticated
USING (public.has_role(auth.uid(), 'admin'))
...
CREATE POLICY IF NOT EXISTS "Admins can delete business hours"
ON public.business_hours
FOR DELETE
TO authenticated
USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY IF NOT EXISTS "Admins can insert rooms"
ON public.rooms
FOR INSERT
TO authenticated
WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY IF NOT EXISTS "Admins can update rooms"
ON public.rooms
FOR UPDATE
TO authenticated
USING (public.has_role(auth.uid(), 'admin'))
...
CREATE POLICY IF NOT EXISTS "Admins can delete rooms"
ON public.rooms
FOR DELETE
TO authenticated
USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY IF NOT EXISTS "Anyone can view promotions"
...
-- 5) Booking status enum and bookings table
DO $$
BEGIN
...
-- Only admins can view/modify bookings; anyone can create pending booking
CREATE POLICY IF NOT EXISTS "Admins can select bookings"
ON public.bookings
FOR SELECT
TO authenticated
USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY IF NOT EXISTS "Admins can insert bookings"
ON public.bookings
FOR INSERT
TO authenticated
WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY IF NOT EXISTS "Anyone can create pending bookings"
ON public.bookings
FOR INSERT
TO anon
WITH CHECK (
...
-- 6) Seed default business hours (Sun-Thu 18-24, Fri 10-15, Sat closed)
